...
{ 
  ...
  if (myFile)
  {
    ...
    Watchdog=180; // Have encountered times where very log log files can cause resets
...
void HandleNotFound()
{
    ...
    {
      if (Uri.indexOf("php")>=0)     { LogString(Uri+" ("+server.client().remoteIP().toString()+") - Ignoring request\n",1); return; }      // Anti hacking precaution. Ignore any request looking for anything involving php
...
void HandleAdminPage() // Request for Admin Page
{
  ...
  if (IsAuthenticated() && LoginTimeout>0)
  {
    ...
    Content = "";
    Content += F("<div class='panel panel-default'><div class='panel-heading'>Set Time/Date</div><div class='panel-body'><form action='/admin.htm' method='POST'><p>");
    Content += String(F("<input type='text' size=4 name='Year' value='"))+String(now.year())+String(F("'> Year<br>"));
    ...
    Content += F("<input type='submit' size=4 name='SUBMIT' value='Submit'></p></form></div>");
    ...
    Content += F("<input type='submit' size=4 name='SUBMIT' value='Submit'></p></form>");
